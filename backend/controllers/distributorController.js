...
/**
 * Helper to build the query matching beneficiaries assigned or relevant to this distributor.
 * Respects strict district and taluk boundaries so Distributor A cannot see Distributor B's beneficiaries.
 */
function buildDistributorBeneficiaryQuery(distributor, reqUserId) {
  const orConditions = [
    { assignedDistributor: distributor._id },
    { assignedDistributor: reqUserId },
  ];

  if (distributor && distributor.district) {
    const escapedDistrict = distributor.district.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const distRegex = new RegExp(`^${escapedDistrict}$`, 'i');

    if (distributor.taluk && distributor.taluk.trim()) {
      const escapedTaluk = distributor.taluk.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const talukRegex = new RegExp(`^${escapedTaluk}$`, 'i');
      orConditions.push({
        $or: [{ assignedDistributor: null }, { assignedDistributor: { $exists: false } }],
        district: distRegex,
        taluk: talukRegex,
      });
    } else {
...
const getAssignedBeneficiaries = async (req, res) => {
  try {
    ...
    const query = buildDistributorBeneficiaryQuery(distributor, req.user.id);

    const { status, eligibleForAllocation, search } = req.query;
    if (status && status !== 'ALL') {
      if (status === 'Active' || status === 'Approved') {
        query.status = { $in: ['Active', 'Approved'] };
      } else if (status === 'Pending Distributor Review') {
        query.status = 'Pending';
        query.submittedToAdmin = { $ne: true };
      } else if (status === 'Submitted for Admin Review') {
        query.status = 'Pending';
        query.submittedToAdmin = true;
      } else {
        query.status = status;
      }
    } else if (eligibleForAllocation === 'true' || eligibleForAllocation === true) {
      query.status = { $in: ['Active', 'Approved'] };
    }
...
const submitBeneficiaryToAdmin = async (req, res) => {
  try {
    ...
    const baseQuery = buildDistributorBeneficiaryQuery(distributor, distributorId);
    ...
    beneficiary.assignedDistributor = distributor._id;
...
const searchBeneficiaryByRationCard = async (req, res) => {
  try {
    ...
    const baseQuery = buildDistributorBeneficiaryQuery(distributor, req.user.id);
    const beneficiary = await Beneficiary.findOne({
...
const allocateRation = async (req, res) => {
  try {
    ...
    const rawRice = riceAllocated !== undefined ? riceAllocated : (rice !== undefined ? rice : (req.body.riceQuantity !== undefined ? req.body.riceQuantity : req.body.riceQuota));
    const rawOil = oilAllocated !== undefined ? oilAllocated : (oil !== undefined ? oil : (req.body.oilQuantity !== undefined ? req.body.oilQuantity : req.body.oilQuota));

    // Reject empty values where allocation is required
    if (
      rawRice === undefined ||
      rawRice === null ||
      (typeof rawRice === 'string' && rawRice.trim() === '') ||
      rawOil === undefined ||
      rawOil === null ||
      (typeof rawOil === 'string' && rawOil.trim() === '')
    ) {
      return res.status(400).json({
        success: false,
        message: 'Please provide valid non-empty quantities for riceAllocated and oilAllocated',
      });
    }

    const finalRice = Number(rawRice);
    const finalOil = Number(rawOil);
    ...
    if (
      ...
      finalRice < 0 ||
      finalOil < 0 ||
      (finalRice === 0 && finalOil === 0)
    ) {
      return res.status(400).json({
        success: false,
        message: 'Please provide valid positive numbers for riceAllocated and oilAllocated',
      });
    }
    ...
    // Ensure beneficiary is assigned to this distributor (or within their FPS district if previously unassigned)
    const beneficiary = await Beneficiary.findOne({
    ...
    if (!beneficiary) {
      return res.status(404).json({
        success: false,
        message: 'Beneficiary not found or not assigned to your Fair Price Shop jurisdiction',
      });
    }

    // Ensure beneficiary is eligible for allocation according to existing workflow (Active or Approved)
    if (beneficiary.status !== 'Active' && beneficiary.status !== 'Approved') {
      return res.status(400).json({
        success: false,
        message: `Beneficiary is not eligible for allocation (current status: ${beneficiary.status}). Only approved beneficiaries who completed the review workflow can receive monthly ration allocations.`,
      });
    }

    // Bind assignedDistributor to this distributor if it was unassigned
    if (!beneficiary.assignedDistributor) {
      beneficiary.assignedDistributor = req.user.id;
      await beneficiary.save();
    }
    ...
    // Also synchronize beneficiary's riceQuota and oilQuota to preserve exact decimal values throughout the system
    beneficiary.riceQuota = finalRice;
    beneficiary.oilQuota = finalOil;
    await beneficiary.save();
    ...
    if (finalStatus === 'Collected') {
      ...
      if (inv) {
        inv.riceStock = Math.max(0, Math.round((inv.riceStock - finalRice) * 1000) / 1000);
        inv.oilStock = Math.max(0, Math.round((inv.oilStock - finalOil) * 1000) / 1000);
...
const updateAllocation = async (req, res) => {
  try {
    ...
    const rawRice = riceAllocated !== undefined ? riceAllocated : rice;
    const rawOil = oilAllocated !== undefined ? oilAllocated : oil;

    if (rawRice !== undefined) {
      if (rawRice === null || (typeof rawRice === 'string' && rawRice.trim() === '')) {
        return res.status(400).json({ success: false, message: 'Rice quantity cannot be empty' });
      }
      const numRice = Number(rawRice);
      if (isNaN(numRice) || numRice < 0) {
        return res.status(400).json({ success: false, message: 'Invalid rice quantity' });
      }
      allocation.riceAllocated = numRice;
    }

    if (rawOil !== undefined) {
      if (rawOil === null || (typeof rawOil === 'string' && rawOil.trim() === '')) {
        return res.status(400).json({ success: false, message: 'Oil quantity cannot be empty' });
      }
      const numOil = Number(rawOil);
      if (isNaN(numOil) || numOil < 0) {
        return res.status(400).json({ success: false, message: 'Invalid oil quantity' });
      }
      allocation.oilAllocated = numOil;
    }
    ...
    // Synchronize beneficiary quotas as well
    if (rawRice !== undefined || rawOil !== undefined) {
      const ben = await Beneficiary.findById(allocation.beneficiary);
      if (ben) {
        if (rawRice !== undefined) ben.riceQuota = allocation.riceAllocated;
        if (rawOil !== undefined) ben.oilQuota = allocation.oilAllocated;
        await ben.save();
      }
    }
...
const getTodayTransactions = async (req, res) => {
  try {
    ...
    const totalRiceDispensed = transactions.reduce(
      (acc, t) => acc + (t.status === 'Successful' ? (t.riceDispensed || 0) : 0),
      0
    );
    const totalOilDispensed = transactions.reduce(
      (acc, t) => acc + (t.status === 'Successful' ? (t.oilDispensed || 0) : 0),
      0
    );
    ...
    return res.status(200).json({
      ...
      totals: {
        riceDispensed: Math.round(totalRiceDispensed * 1000) / 1000,
        oilDispensed: Math.round(totalOilDispensed * 1000) / 1000,
      },
      data: transactions,
    });
  } catch (error) {
...
const getMonthlyTransactions = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      count: transactions.length,
      totals: {
        riceDispensed: Math.round(totalRice * 1000) / 1000,
        oilDispensed: Math.round(totalOil * 1000) / 1000,
      },
      data: transactions,
    });
  } catch (error) {
