...
const beneficiaryLogin = async (req, res) => {
  try {
    ...
    if (!rawRationCard || !password) {
      return res.status(400).json({
        success: false,
        message: 'Please provide both Ration Card Number / Mobile Number and password',
      });
    }
    ...
    if (!beneficiary) {
      return res.status(401).json({
        success: false,
        message: 'Invalid Ration Card Number / Mobile Number or password',
      });
    }
...
const beneficiaryRegister = async (req, res) => {
  try {
    ...
    // Validate required fields (Ration Card Number is NOT required during registration - assigned later by Admin)
    if (
      !rawFullName ||
      !rawMobileNumber ||
      !password ||
      ...
      return res.status(400).json({
        success: false,
        message: 'Please provide all required fields (fullName, mobileNumber, password, district, taluk)',
      });
    }

    const trimmedRationCard = rawRationCardNumber && String(rawRationCardNumber).trim()
      ? String(rawRationCardNumber).trim().toUpperCase()
      : undefined;

    // Check if duplicate Ration Card Number exists (only if provided)
    if (trimmedRationCard) {
    ...
    const trimmedMobile = String(rawMobileNumber).trim();
    const existingMobile = await Beneficiary.findOne({
      mobileNumber: trimmedMobile,
    });
    if (existingMobile) {
      return res.status(400).json({
        success: false,
        message: 'A beneficiary with this Mobile Number already exists',
      });
    }
    ...
    if (!distributorToAssign && district) {
      const escapedDist = String(district).trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const distRegex = new RegExp(`^${escapedDist}$`, 'i');
      if (taluk && String(taluk).trim()) {
        const escapedTaluk = String(taluk).trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const talukRegex = new RegExp(`^${escapedTaluk}$`, 'i');
        const matchedDist = await Distributor.findOne({
          district: distRegex,
          $or: [{ taluk: talukRegex }, { taluk: null }, { taluk: '' }, { taluk: { $exists: false } }],
        });
        if (matchedDist) {
          distributorToAssign = matchedDist._id;
        }
      } else {
        const matchedDist = await Distributor.findOne({ district: distRegex });
        if (matchedDist) {
          distributorToAssign = matchedDist._id;
        }
      }
    ...
    // Newly registered beneficiaries must have an unassigned quota (0) until authorized by Distributor Monthly Allocation
    const familyCount = Math.max(1, Number(familyMemberCount) || 1);
    const finalRiceQuota = riceQuota > 0 ? riceQuota : 0;
    const finalOilQuota = oilQuota > 0 ? oilQuota : 0;
