...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (usersTableBody) {
      ...
      if (filtered.length === 0) {
        ...
        filtered.forEach((user) => {
          ...
          const cardDisplay = user.rationCardNumber
            ? `<strong style="color: #2563EB;">${user.rationCardNumber}</strong>`
            : `<span style="color: #D97706; font-size: 0.78rem; font-style: italic; background: #FEF3C7; padding: 2px 6px; border-radius: 4px;">Not Assigned</span>`;
    ...
    usersTableBody.addEventListener('click', async (e) => {
      ...
      if (btn.classList.contains('view-user-btn')) {
        if (viewUserModalContent && viewUserModal) {
          ...
          const userCardStatus = user.rationCardNumber
            ? `<strong style="color: #2563EB; font-size: 1rem;">${user.rationCardNumber}</strong>`
            : `<span style="color: #D97706; font-size: 0.85rem; font-style: italic; background: #FEF3C7; padding: 3px 8px; border-radius: 4px;">Not Assigned Yet (Pending Admin Assignment)</span>`;
          ...
          viewUserModalContent.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; font-size: 0.9rem; margin-bottom: 12px;">
              ...
              <div><strong>Ration Card # / User ID:</strong><br>${userCardStatus}</div>
              ...
              <!-- Admin Manual Ration Card Assignment Section -->
              <div style="grid-column: span 2; background: #F8FAFC; padding: 14px; border-radius: 8px; border: 1px solid #E2E8F0; margin-top: 6px;">
                <div style="font-weight: 600; font-size: 0.9rem; color: #0F172A; margin-bottom: 4px;">
                  Assign / Update Ration Card Number (RFID Integration)
                </div>
                <div style="font-size: 0.8rem; color: #64748B; margin-bottom: 10px;">
                  Beneficiaries register without needing a Ration Card Number. As Admin, you can manually enter or assign their official Ration Card Number here to link with their account.
                </div>
                <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                  <input 
                    type="text" 
                    id="adminAssignRationCardInput" 
                    class="form-input" 
                    style="padding: 8px 12px; font-size: 0.875rem; max-width: 260px; text-transform: uppercase;" 
                    placeholder="e.g. RC98765432" 
                    value="${user.rationCardNumber || ''}"
                  />
                  <button 
                    type="button" 
                    id="saveRationCardBtn" 
                    data-id="${userId}" 
                ...
                <div id="assignRationCardStatus" style="margin-top: 6px; font-size: 0.82rem;"></div>
              </div>
            </div>
          `;

          // Setup Save Ration Card Handler
          const saveBtn = viewUserModalContent.querySelector('#saveRationCardBtn');
          const cardInput = viewUserModalContent.querySelector('#adminAssignRationCardInput');
          const statusDiv = viewUserModalContent.querySelector('#assignRationCardStatus');

          if (saveBtn && cardInput) {
            saveBtn.addEventListener('click', async () => {
              const enteredCard = cardInput.value.trim().toUpperCase();
              saveBtn.disabled = true;
              saveBtn.textContent = 'Saving...';
              if (statusDiv) statusDiv.textContent = '';

              try {
                const res = await adminApi.updateBeneficiary(userId, {
                  rationCardNumber: enteredCard || null,
                });
                showToast(res.message || 'Ration Card Number updated successfully.');
                if (statusDiv) {
                  statusDiv.style.color = '#15803D';
                  statusDiv.textContent = enteredCard 
                    ? `✓ Ration Card Number ${enteredCard} assigned successfully!`
                    : `✓ Ration Card Number cleared.`;
                }
                user.rationCardNumber = enteredCard || undefined;
                renderUsersTable();
                if (typeof loadBeneficiaries === 'function') loadBeneficiaries();
              } catch (err) {
                if (statusDiv) {
                  statusDiv.style.color = '#DC2626';
                  statusDiv.textContent = `✗ ${err.message || 'Failed to update Ration Card Number'}`;
                }
                showToast(`Error: ${err.message}`);
              } finally {
                saveBtn.disabled = false;
                saveBtn.textContent = 'Save / Assign Card #';
              }
            });
          }
      ...
      if (btn.classList.contains('approve-user-btn')) {
        const cardDisplay = user.rationCardNumber ? `Card: ${user.rationCardNumber}` : 'Card: Pending Assignment';
        if (confirm(`Approve beneficiary application for ${user.fullName} (${cardDisplay}) with Rice: ${user.riceQuota || 0}KG, Oil: ${user.oilQuota || 0}L?`)) {
          try {
            const res = await adminApi.approveBeneficiary(userId);
            showToast(res.message || `User ${user.fullName} approved successfully.`);
            addSystemNotification('User Approved', `Approved ration quota for ${user.fullName} (${cardDisplay}): ${user.riceQuota || 0}KG Rice, ${user.oilQuota || 0}L Oil.`);
            await loadUsers();
            if (typeof loadBeneficiaries === 'function') loadBeneficiaries();
          } catch (err) {
            showToast(`Approve error: ${err.message}`);
      ...
      if (btn.classList.contains('reject-user-btn')) {
        const cardDisplay = user.rationCardNumber ? `Card: ${user.rationCardNumber}` : 'Card: Pending Assignment';
        if (confirm(`Reject beneficiary application for ${user.fullName} (${cardDisplay})?`)) {
          try {
            const res = await adminApi.rejectBeneficiary(userId);
            showToast(res.message || `User ${user.fullName} application rejected.`);
            addSystemNotification('User Rejected', `Rejected application for ${user.fullName} (${cardDisplay}).`);
            await loadUsers();
            if (typeof loadBeneficiaries === 'function') loadBeneficiaries();
          } catch (err) {
            showToast(`Reject error: ${err.message}`);
  ...
  if (restockForm) {
    restockForm.addEventListener('submit', async (e) => {
      ...
      const qty = parseFloat(document.getElementById('restockAmountInput').value);
  ...
  if (dispatchForm) {
    dispatchForm.addEventListener('submit', async (e) => {
      ...
      const quantity = parseFloat(document.getElementById('dispatchQuantityInput').value);
