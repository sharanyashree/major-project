...
const approveBeneficiary = async (req, res) => {
  try {
    ...
    if (req.body && req.body.assignedDistributor) {
      beneficiary.assignedDistributor = req.body.assignedDistributor;
    } else if (!beneficiary.assignedDistributor && beneficiary.district) {
      const distRegex = new RegExp(`^${beneficiary.district.trim()}$`, 'i');
      let matchedDist = null;
      if (beneficiary.taluk) {
        const talukRegex = new RegExp(`^${beneficiary.taluk.trim()}$`, 'i');
      ...
      if (matchedDist) {
        beneficiary.assignedDistributor = matchedDist._id;
      }
    }
...
/**
 * @desc    Get single beneficiary details by ID
 * @route   GET /api/admin/beneficiaries/:id
 * @access  Private (Admin)
 */
const getBeneficiaryById = async (req, res) => {
  try {
    ...
    const beneficiary = await Beneficiary.findById(id)
      .select('-password')
      .populate('assignedDistributor', 'name distributorId fpsCode district taluk mobileNumber');
    ...
    const allocations = await Allocation.find({ beneficiary: id }).sort({ year: -1, createdAt: -1 });
...
/**
 * @desc    Update beneficiary details (e.g. manually assign Ration Card Number, quotas, RFID)
 * @route   PUT /api/admin/beneficiaries/:id
 * @access  Private (Admin)
 */
const updateBeneficiary = async (req, res) => {
  try {
    ...
    const { rationCardNumber, rfidUid, riceQuota, oilQuota, status, fullName, mobileNumber } = req.body;

    // Handle manual entry / assignment of Ration Card Number by Admin
    if (rationCardNumber !== undefined) {
      const trimmedCard = String(rationCardNumber).trim().toUpperCase();
      if (trimmedCard) {
        // Check for duplicate in another beneficiary record
        const duplicate = await Beneficiary.findOne({
          _id: { $ne: id },
          rationCardNumber: trimmedCard,
        });
        if (duplicate) {
          return res.status(400).json({
            success: false,
            message: `Ration Card Number '${trimmedCard}' is already assigned to beneficiary ${duplicate.fullName}`,
          });
        }
        beneficiary.rationCardNumber = trimmedCard;
      } else {
        beneficiary.rationCardNumber = undefined;
      }
    }

    // Handle RFID UID assignment
    if (rfidUid !== undefined) {
      const trimmedRfid = String(rfidUid).trim();
      if (trimmedRfid) {
        const duplicateRfid = await Beneficiary.findOne({
          _id: { $ne: id },
          rfidUid: trimmedRfid,
        });
        if (duplicateRfid) {
          return res.status(400).json({
            success: false,
            message: `RFID UID '${trimmedRfid}' is already assigned to another beneficiary`,
          });
        }
        beneficiary.rfidUid = trimmedRfid;
      }
    }

    if (riceQuota !== undefined && !isNaN(Number(riceQuota))) {
      beneficiary.riceQuota = Math.max(0, Number(riceQuota));
    }
    if (oilQuota !== undefined && !isNaN(Number(oilQuota))) {
      beneficiary.oilQuota = Math.max(0, Number(oilQuota));
    }
    if (status !== undefined) {
      beneficiary.status = status;
    }
    if (fullName !== undefined && String(fullName).trim()) {
      beneficiary.fullName = String(fullName).trim();
    }
    if (mobileNumber !== undefined && String(mobileNumber).trim()) {
      beneficiary.mobileNumber = String(mobileNumber).trim();
    }
