...
const getDashboardSummary = async (req, res) => {
  try {
    ...
    // Fetch Current Month's Allocation or most recent allocation
    let currentAllocation = await Allocation.findOne({
    ...
    if (!currentAllocation) {
      currentAllocation = await Allocation.findOne({
        beneficiary: beneficiaryId,
      })
    ...
    const riceAllocated = currentAllocation != null
      ? currentAllocation.riceAllocated
      : (beneficiary.riceQuota != null ? beneficiary.riceQuota : 0);
    const oilAllocated = currentAllocation != null
      ? currentAllocation.oilAllocated
      : (beneficiary.oilQuota != null ? beneficiary.oilQuota : 0);
    const allocMonth = currentAllocation ? currentAllocation.month : currentMonth;
    const allocYear = currentAllocation ? currentAllocation.year : currentYear;
    const collStatus = currentAllocation
      ? currentAllocation.collectionStatus
      : ((riceAllocated > 0 || oilAllocated > 0) ? 'Pending' : 'Not Allocated');
...
const getCurrentAllocation = async (req, res) => {
  try {
    ...
    const beneficiary = await Beneficiary.findById(req.user.id)
      .select('riceQuota oilQuota assignedDistributor')
      .populate('assignedDistributor', 'name distributorId fpsCode mobileNumber district taluk');

    if (!allocation) {
      const benRice = beneficiary ? (beneficiary.riceQuota || 0) : 0;
      const benOil = beneficiary ? (beneficiary.oilQuota || 0) : 0;
      return res.status(200).json({
        success: true,
        ...
        data: {
          ...
          distributor: beneficiary?.assignedDistributor || null,
        },
      });
    }
